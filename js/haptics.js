...
class HapticFeedback {
    constructor() {
        ...
        if (this.canVibrate) {
            ...
            console.log('✓ Haptics available (native vibration)');
        } else if (this.isIOS) {
            this.backend = 'audio';
            console.log('✓ Haptics available (AudioContext fallback for iOS)');
        } else {
    ...
    async initialize() {
        ...
        if (this.canVibrate) {
            try {
                ...
                console.log('Vibration test failed, falling back to audio');
                this.backend = 'audio';
            }
        }
        ...
        if (this.backend === 'audio') {
            if (typeof window.audioContext === 'undefined' || !window.audioContext) {
                console.log('⚠ AudioContext not available for haptic simulation');
                this.backend = 'none';
        ...
        this.initialized = true;
        console.log(`✓ Haptics initialized using ${this.backend} backend`);
        return true;
    }
    ...
    vibrateAudio(pattern) {
        ...
        try {
            ...
            console.log('Audio haptic failed:', e);
        }
    }
