...
/**
 * A single bonus orb spawned during rainbow bonus mode.
 * Handles its own per-frame animation; lifecycle managed by BonusOrbManager.
 */
class BonusOrb {
    /**
     * @param {number} x - World X position (lane center)
     * @param {number} y - World Y position (height lane)
    ...
    /**
     * Animate the orb: move, bob, pulse emissive intensity, and rotate.
     * @param {number} delta - Seconds since last frame
     * @param {number} elapsed - Total seconds elapsed since game start
     * @param {number} moveAmount - Distance to move toward player this frame
     */
...
const BonusOrbManager = {
    ...
    /**
     * Build a bonus orb mesh group using cached geometries and add it to the scene.
     * @returns {THREE.Group} The orb group (already added to scene)
     */
    createOrbMesh() {
        const group = new THREE.Group();

        // Bright, consistent orb visuals for clarity on rainbow floor
        const coreGeo = GeometryCache.get('sphere', 0.36, 16, 16);
        ...
        // Brighter outer glow
        const glowGeo = GeometryCache.get('sphere', 0.55, 12, 12);
        ...
        // Consistent ring
        const ringGeo = GeometryCache.get('torus', 0.45, 0.03, 8, 24);
    ...
    /**
     * Spawn new orbs ahead of the player, animate all active orbs, apply magnet pull,
     * and despawn orbs that have passed behind the player.
     * No-ops (and clears all orbs) if bonus mode is not active.
     * @param {number} delta - Seconds since last frame
     * @param {number} elapsed - Total seconds elapsed since game start
     */
    update(delta, elapsed) {
        ...
        for (let i = this.bonusOrbs.length - 1; i >= 0; i--) {
            ...
            if (orb.group.position.z < bonusDespawnZ) {
                ObstacleManager.disposeObject(orb.group);
    ...
    /**
     * Spawn a single bonus orb at the given Z position using the current lane/height pattern.
     * Advances lane and height indices with bounce (zigzag) motion.
     * @param {number} zBase - World Z position to spawn at
     * @returns {number} Pattern length offset (always 0 for single-orb patterns)
     */
    ...
    /**
     * Test proximity of each bonus orb against the player in 3D; collect on contact.
     * Uses both distance threshold and crossing-plane detection for reliability.
     */
    checkCollection() {
    ...
    /**
     * Remove and dispose all active bonus orbs. Reset spawn state and pattern indices.
     * Called on game restart, menu return, or when bonus mode ends mid-frame.
     */
    reset() {
