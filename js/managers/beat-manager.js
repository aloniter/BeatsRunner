...
const BeatManager = {
    ...
    /**
     * Detect beat crossings, update beat intensity for visual pulsing, and drive visuals.
    ...
    /**
     * Get timing accuracy relative to the nearest beat
     * Returns rating and multiplier based on how close the current time is to a beat
     */
    getTimingAccuracy(elapsed) {
        const beatInterval = CONFIG.BEAT_INTERVAL;
        const timeSinceBeat = elapsed - this.lastBeatTime;
        const halfBeat = beatInterval / 2;

        // Normalize: 0 = on beat, 1 = mid-beat
        const distToBeat = Math.min(timeSinceBeat, beatInterval - timeSinceBeat);
        const beatDistance = distToBeat / halfBeat;

        if (beatDistance < 0.15) return { rating: 'PERFECT', multiplier: 3, color: '#ffff00' };
        if (beatDistance < 0.35) return { rating: 'GOOD', multiplier: 2, color: '#00ff88' };
        if (beatDistance < 0.65) return { rating: 'OK', multiplier: 1.5, color: '#00ccff' };
        return { rating: 'MISS', multiplier: 1, color: '#888888' };
    },

    /**
     * Fired once per beat: pulses the beat indicator, micro-shakes the camera,
     * plays the beat sound, flashes the screen, ramps game speed, and drives
     * disco ball and rainbow orb beat responses.
     */
    onBeat() {
    ...
    /**
     * Apply current beat intensity to floor tile emissive, player glow opacity,
     * disco ball components, and rainbow orb color transitions. Called every frame by update().
     */
    updateVisuals() {
    ...
    /**
     * Apply a brief upward micro-jitter to the camera on each beat.
     * Uses a 40 ms timeout to snap the camera back to its resting Y position (6).
     */
    shakeCamera() {
    ...
    /**
     * Reset all beat state to initial values. Also resets rainbow orb color index and
     * transition progress. Called on game restart and when returning to main menu.
     */
    reset() {
        this.lastBeatTime = 0;
        this.beatCount = 0;
